...
import { DbLiveSession, DbParticipant } from '../types/index.js';

const router = Router();
// NOTE: Global router.use(requireAuth) has been removed to allow unauthenticated guest students to join and play live sessions.
// requireAuth is applied explicitly to teacher-only management routes below.
...
router.get('/reports', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - Reports]', err);
    res.status(500).json({ error: err?.message || 'Failed to fetch session reports' });
  }
});
...
router.get('/history', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - History]', err);
...
router.post('/', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - Create]', err);
...
router.get('/by-code/:joinCode', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - ByCode]', err);
    res.status(500).json({ error: err?.message || 'Failed to lookup room code' });
  }
});
...
router.get('/:id', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - Get ID]', err);
    res.status(500).json({ error: err?.message || 'Failed to fetch session' });
  }
});

// POST /api/sessions/:id/join — student joins session (public guest route)
router.post('/:id/join', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - Join]', err);
    res.status(500).json({ error: err?.message || 'Failed to join live session' });
  }
});

// GET /api/sessions/:id/play-data — student's view of session (public guest route)
router.get('/:id/play-data', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - PlayData]', err);
    res.status(500).json({ error: err?.message || 'Failed to fetch session play data' });
  }
});

// POST /api/sessions/:id/start (teacher route)
router.post('/:id/start', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - Start]', err);
    res.status(500).json({ error: err?.message || 'Failed to start session' });
  }
});

// POST /api/sessions/:id/next — advance question or finish (teacher route)
router.post('/:id/next', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    if (!res.headersSent) res.status(500).json({ error: err?.message || 'Failed to advance question' });
  }
});

// POST /api/sessions/:id/finish (teacher route)
router.post('/:id/finish', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    console.error('[Sessions Route Error - Finish]', err);
    res.status(500).json({ error: err?.message || 'Failed to finish session' });
  }
});

// POST /api/sessions/:id/submit — student submits answer (public guest route)
  ...
  try {
    ...
    try {
      ...
      if (isTimedScoring && sessionRow?.currentQuestionStartedAt) {
        ...
        if ((now - started) / 1000 > timeLimit + 1) {
    ...
    console.error('[Sessions Route Error - Submit]', err);
    res.status(500).json({ error: err?.message || 'Failed to submit answer' });
  }
});

// POST /api/sessions/:id/flag-cheat (public guest route)
router.post('/:id/flag-cheat', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - FlagCheat]', err);
    res.status(500).json({ error: err?.message || 'Failed to flag violation' });
  }
});

// POST /api/sessions/:id/kick (teacher route)
router.post('/:id/kick', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - Kick]', err);
    res.status(500).json({ error: err?.message || 'Failed to kick player' });
  }
});

// DELETE /api/sessions/:id/leave (public guest route)
router.delete('/:id/leave', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - Leave]', err);
    res.status(500).json({ error: err?.message || 'Failed to leave session' });
  }
});

// GET /api/sessions/:id/submission-count (public guest route)
router.get('/:id/submission-count', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - SubmissionCount]', err);
    res.status(500).json({ error: err?.message || 'Failed to fetch submission count' });
  }
});

// GET /api/sessions/:id/answered — student's already-answered question IDs (public guest route)
router.get('/:id/answered', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - Answered]', err);
    res.status(500).json({ error: err?.message || 'Failed to fetch answered questions' });
  }
});

// GET /api/sessions/:id/state — single source of truth for participant on reload (public guest route)
router.get('/:id/state', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - State]', err);
    res.status(500).json({ error: err?.message || 'Failed to fetch session state' });
  }
});

// DELETE /api/sessions/:id — teacher deletes a finished session report (teacher route)
router.delete('/:id', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - Delete Session]', err);
    res.status(500).json({ error: err?.message || 'Failed to delete session' });
  }
});

// GET /api/sessions/:id/report — full analytics (teacher route)
router.get('/:id/report', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - Full Report]', err);
    res.status(500).json({ error: err?.message || 'Failed to fetch session report' });
  }
});

// GET /api/sessions/:id/result — participant-facing final result view (public guest route)
router.get('/:id/result', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - Result]', err);
    res.status(500).json({ error: err?.message || 'Failed to fetch session results' });
  }
});

// GET /api/sessions/:id/leaderboard (public guest route)
router.get('/:id/leaderboard', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Sessions Route Error - Leaderboard]', err);
    res.status(500).json({ error: err?.message || 'Failed to fetch leaderboard' });
  }
});

// GET /api/sessions/:id/report.xlsx — Export full analytics as Excel (teacher route)
router.get('/:id/report.xlsx', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const [sessionRows] = await pool.query<any[]>(
      'SELECT ls.*, q.title AS quizTitle FROM LiveSession ls JOIN Quiz q ON q.id = ls.quizId WHERE ls.id = ? AND ls.deletedAt IS NULL',
    ...
    console.error('[Sessions Route Error - Excel Export]', err);
    res.status(500).send(err?.message || 'Failed to generate Excel report');
  }
});
