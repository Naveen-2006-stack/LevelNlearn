...
export const ActiveQuestionCard = ({
  ...
  const [questionTimeLimit, setQuestionTimeLimit] = useState(timeLimit);
  ...
  const handleConfirmSubmit = useCallback(async (overrideIdx?: number) => {
    // overrideIdx === -1 is the "time expired, no answer" sentinel — mark as submitted
    // locally but do NOT send an API call (nothing to submit).
    if (overrideIdx === -1) {
  ...
  useEffect(() => {
    if (lastQuestionIdRef.current === questionId) return;
  ...
  }, [questionId, question, timeLimit, options]);

  useEffect(() => {
    if (hasSubmitted || isRevealed || !timedScoring) return;
  ...
  }, [handleConfirmSubmit, hasSubmitted, isRevealed, timedScoring]);

  useEffect(() => {
    if (isRevealed && !hasSubmitted) setHasSubmitted(true);
  ...
  const timeFraction = questionTimeLimit > 0 ? timeLeft / questionTimeLimit : 0;
  const timerColor =
  ...
  return (
    ...
    >
      {/* Timer bar — hidden when timed scoring is off */}
      {timedScoring && (
      ...
      <div className="p-6 lg:p-10">
        <div className="flex justify-between items-center mb-8">
          {/* Timer number — only shown when timed scoring is on */}
          {timedScoring && (
            <motion.div
        ...
        {imageUrl && (
          <div className="mb-6 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
            <img src={imageUrl} alt="Question visual" className="w-full max-h-80 object-contain" loading="lazy" />
          </div>
        )}
        ...
        <AnimatePresence mode="wait">
          {showReveal ? (
            ...
            >
              ...
              <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-1">Answer Locked</h3>
                ...
                {shuffledOptions.map((opt, idx) => {
                  ...
                  return (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.97 }}
                      onClick={() => {
                        if (isTrueFalse) { setSelectedIndices([idx]); return; }
                        if (!isMultiSelect) { setSelectedIndices([idx]); return; }
                        setSelectedIndices((prev) =>
                          prev.includes(idx) ? prev.filter((v) => v !== idx) : [...prev, idx]
                        );
                      }}
                      className={cn(
                        'relative w-full text-left rounded-[1.5rem] font-bold transition-all duration-200 border-4 min-h-[64px] sm:min-h-[80px]',
                        isTrueFalse ? 'p-8 text-3xl' : 'p-5 text-lg',
                        color.bg, color.hover, color.text, color.depth,
                        isSelected
                    ...
                    >
                      ...
                      {isGhostReveal && (
                        <div aria-hidden="true" className="absolute top-3 right-3 w-1 h-1 rounded-full bg-white/40 pointer-events-none" title="Ghost: correct answer" />
                      )}
                    </motion.button>
                  );
                })}
