...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen transition-colors overflow-hidden relative">
        ...
        <div className="flex items-center gap-3">
          {mounted ? (
            <button
              ...
              className="p-2 rounded-full text-slate-500 hover:text-indigo-600 dark:text-slate-300 dark:hover:text-indigo-400 transition-colors hover:bg-indigo-50 dark:hover:bg-indigo-500/10"
              aria-label="Toggle Theme"
            >
              {resolvedTheme === 'dark' ? <Sun size={18} /> : <Moon size={18} />}
            </button>
          ) : null}
          <Link to="/login" className="text-slate-600 dark:text-slate-300 font-semibold hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors px-4 py-2">
            Sign In
          </Link>
        </div>
      </nav>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 pt-20 pb-32 flex flex-col items-center text-center">
        ...
        <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.5 }} className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-32 text-left w-full">
          <div className="bg-white/70 dark:bg-slate-900/50 p-8 rounded-3xl border border-white/20 dark:border-slate-800/50 backdrop-blur-xl">
            ...
            <p className="text-slate-500 dark:text-slate-400">Powered by Pusher WebSockets. Broadcast questions to thousands of students instantly.</p>
          </div>
          <div className="bg-white/70 dark:bg-slate-900/50 p-8 rounded-3xl border border-white/20 dark:border-slate-800/50 backdrop-blur-xl">
