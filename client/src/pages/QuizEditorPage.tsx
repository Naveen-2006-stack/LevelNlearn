...
const normalizeQuestionType = (q: any): QuestionType => {
  if (q?.question_type === 'true_false' || q?.question_type === 'multi_select' || q?.question_type === 'mcq') return q.question_type;
  const correctCount = Array.isArray(q?.options) ? q.options.filter((o: any) => !!o?.is_correct).length : 0;
...
export default function QuizEditorPage() {
  ...
  const [quiz, setQuiz] = useState<Quiz>({ id: quizId!, title: 'Loading...', description: '', timer_based_marking: true, test_mode: false });
  ...
  const fetchQuizData = async () => {
    try {
      ...
      setQuiz({
        ...
        timer_based_marking: Boolean(data.timer_based_marking ?? data.timerBasedMarking ?? true),
        test_mode: Boolean(data.test_mode ?? data.testMode ?? false),
      });
      ...
      setSaveStatus({ type: 'error', message: err?.response?.data?.error || 'Error loading quiz' });
    }
  };
  ...
  const setQuestionType = (qIndex: number, nextType: QuestionType) => {
    ...
    if (nextType === 'true_false') {
      ...
      updated[qIndex] = { ...current, question_type: 'true_false', options: [{ text: 'True', is_correct: trueShouldBeCorrect }, { text: 'False', is_correct: !trueShouldBeCorrect }] };
      ...
      if (!padded.some((o) => o.is_correct)) padded[0].is_correct = true;
      updated[qIndex] = { ...current, question_type: 'multi_select', options: padded };
    } else {
      const prev = current.options;
      const padded = [
        prev[0] ? { ...prev[0] } : { text: '', is_correct: true },
      ...
      updated[qIndex] = { ...current, question_type: 'mcq', options: padded.map((opt, idx) => ({ ...opt, is_correct: idx === 0 })) };
    }
  ...
  const deleteQuestion = async (index: number, rawId: string, isNew?: boolean) => {
    if (!isNew) {
      try { await quizApi.deleteQuestion(quizId!, rawId); } catch { /* ignore */ }
    }
  ...
  const saveQuiz = async () => {
    ...
    if (questions.length === 0) {
      setSaveStatus({ type: 'error', message: 'You cannot save an empty quiz. Please add at least one question.' });
      setSaving(false);
      return;
    }
    for (let i = 0; i < questions.length; i++) {
      const q = questions[i];
      if (!q.question_text.trim()) validationErrors.push(`Question ${i + 1}: empty text.`);
      if (q.options.some((opt) => !opt.text.trim())) validationErrors.push(`Question ${i + 1}: empty option.`);
      if (!q.options.some((opt) => !!opt.is_correct)) validationErrors.push(`Question ${i + 1}: no correct answer.`);
    }
    ...
    try {
      await quizApi.update(quizId!, {
        ...
        timer_based_marking: quiz.timer_based_marking,
        test_mode: quiz.test_mode,
        questions,
      });
      setSaveStatus({ type: 'success', message: `Quiz saved! ${questions.length} question${questions.length !== 1 ? 's' : ''} saved.` });
      setTimeout(() => setSaveStatus(null), 4000);

      try {
        await fetchQuizData();
      } catch {
        // Keep the successful save message; the local form already has the latest edits.
      }
    } catch (err: any) {
      setSaveStatus({ type: 'error', message: err?.response?.data?.error || 'Unexpected error saving quiz.' });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-24">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-white/10">
        <Link to="/dashboard" className="flex items-center gap-2 text-slate-500 hover:text-indigo-600 transition-colors">
      ...
      <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-white/10 space-y-6">
        ...
        <div className="flex items-start sm:items-center justify-between gap-4 p-4 bg-gray-50 dark:bg-slate-900/50 rounded-2xl border border-gray-100 dark:border-white/5">
          ...
          >
            <span className={`${quiz.timer_based_marking ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`} />
          </button>
        </div>
        ...
        <div className="flex items-start sm:items-center justify-between gap-4 p-4 bg-gray-50 dark:bg-slate-900/50 rounded-2xl border border-gray-100 dark:border-white/5">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">When on, students cannot see their marks or the final leaderboard</p>
          </div>
          <button
          ...
          >
            <span className={`${quiz.test_mode ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`} />
          </button>
        </div>
      </div>
              ...
              <div className="flex items-start gap-4 mb-6">
                ...
                <div className="flex-1 space-y-4">
                  <div className="flex justify-between items-start gap-4">
                    ...
                    />
                    <button onClick={() => deleteQuestion(qIdx, q.id, q._isNew)} className="p-3 text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-xl transition-colors">
                      <Trash2 size={20} />
                    </button>
                  </div>
                  ...
                  <div className="rounded-2xl border border-dashed border-indigo-200 bg-indigo-50/40 p-4">
                    <div className="flex flex-wrap items-center gap-3">
                      <label className="inline-flex items-center gap-2 cursor-pointer rounded-xl bg-white px-4 py-2 text-sm font-semibold text-indigo-700 border border-indigo-200 hover:bg-indigo-50 transition-colors">
                        ...
                        <input
                          type="file" accept="image/*" className="hidden" disabled={uploadingQuestionId === q.id}
                          onChange={(e) => { const file = e.target.files?.[0]; void uploadQuestionImageLocal(qIdx, file); e.currentTarget.value = ''; }}
                        />
                      </label>
                      {q.image_url && (
                        <button type="button" onClick={() => updateQuestion(qIdx, 'image_url', null)} className="inline-flex items-center gap-1 rounded-xl bg-rose-100 px-3 py-2 text-sm font-semibold text-rose-700 hover:bg-rose-200 transition-colors">
                          <X size={14} /> Remove
                        </button>
                      )}
                    </div>
                    {q.image_url && (
                      <div className="mt-3 overflow-hidden rounded-xl border border-indigo-100 bg-white">
                        <img src={q.image_url} alt={`Question ${qIdx + 1} preview`} className="w-full max-h-64 object-contain" loading="lazy" />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-4">
                    <div className="flex items-center gap-2 bg-gray-50 dark:bg-slate-900 px-4 py-2 rounded-xl border border-gray-200 dark:border-white/10">
                      ...
                      <select value={q.question_type || 'mcq'} onChange={(e) => setQuestionType(qIdx, e.target.value as QuestionType)} className="bg-transparent text-slate-900 dark:text-white font-semibold outline-none cursor-pointer">
                        <option value="mcq" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">Multiple Choice</option>
                        <option value="multi_select" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">Multiple Answer</option>
                        <option value="true_false" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">True / False</option>
                      </select>
                    </div>
                    <div className="flex items-center gap-2 bg-gray-50 dark:bg-slate-900 px-4 py-2 rounded-xl border border-gray-200 dark:border-white/10">
                      <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Time Limit:</span>
                      <select value={q.time_limit} onChange={(e) => updateQuestion(qIdx, 'time_limit', parseInt(e.target.value))} className="bg-transparent text-slate-900 dark:text-white font-semibold outline-none cursor-pointer">
                        {[10, 20, 30, 40, 60, 90].map((t) => <option key={t} value={t} className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">{t}s</option>)}
                      </select>
                    </div>
                    <div className="flex items-center gap-2 bg-gray-50 dark:bg-slate-900 px-4 py-2 rounded-xl border border-gray-200 dark:border-white/10">
                      <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Points:</span>
                      <select value={q.base_points} onChange={(e) => updateQuestion(qIdx, 'base_points', parseInt(e.target.value))} className="bg-transparent text-slate-900 dark:text-white font-semibold outline-none cursor-pointer">
                        {[50, 100, 200].map((p) => <option key={p} value={p} className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">{p}</option>)}
                      </select>
                    </div>
                  </div>
                </div>
              ...
              {q.question_type === 'true_false' ? (
                <div className="ml-9 grid grid-cols-1 md:grid-cols-2 gap-4">
                  {q.options.slice(0, 2).map((opt, oIdx) => {
                    ...
                    return (
                      <button key={oIdx} type="button" onClick={() => setCorrectOption(qIdx, oIdx)}
                        className={`relative w-full p-6 rounded-2xl border-2 text-left transition-all ${isTrue ? 'bg-blue-500 text-white border-blue-400' : 'bg-rose-500 text-white border-rose-400'} ${opt.is_correct ? 'ring-4 ring-white/40 scale-[1.01]' : 'opacity-75 hover:opacity-100'}`}
                      >
                        <div className="flex items-center justify-between">
                ...
                <div className="ml-9 grid grid-cols-1 md:grid-cols-2 gap-4">
                  {q.question_type === 'multi_select' && (
                    <div className="md:col-span-2 mb-1 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
                      Multiple Answer: mark all correct options for this question.
                    </div>
                  )}
                  {q.options.map((opt, oIdx) => (
                    <div key={oIdx} className={`relative flex items-center p-2 rounded-xl border-2 transition-all ${opt.is_correct ? 'border-emerald-500 bg-emerald-50/50 dark:bg-emerald-500/10' : 'border-transparent bg-gray-50 dark:bg-slate-900 focus-within:border-indigo-200 dark:focus-within:border-indigo-500/50'}`}>
                      <button onClick={() => setCorrectOption(qIdx, oIdx)} className={`p-2 rounded-lg transition-colors ${opt.is_correct ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-300 hover:text-slate-500 dark:text-slate-600 dark:hover:text-slate-400'}`} title={opt.is_correct ? 'Marked correct (click to unmark)' : 'Click to mark as correct'}>
                        <CheckCircle2 size={24} className={opt.is_correct ? 'fill-emerald-100 dark:fill-emerald-900/30' : ''} />
                      </button>
                      <input type="text" value={opt.text} onChange={(e) => updateOption(qIdx, oIdx, e.target.value)} placeholder={`Add answer ${oIdx + 1}`} className="flex-1 px-2 py-2 bg-transparent outline-none text-slate-900 dark:text-white font-medium" />
                    </div>
                  ))}
                </div>
              )}
