...
import {
  hostFetchSessionAction,
  hostTouchSessionAction,
  hostStartGameAction,
  hostAdvanceQuestionAction,
  hostTerminateQuizAction,
  hostKickParticipantAction,
  hostFetchViolationsAction,
  hostLogViolationAction,
  hostGetSubmissionCountAction
} from "@/actions/host";
...
export default function HostRoom() {
  ...
  const touchSessionActivity = async () => {
    if (!sessionId) return;
    try {
      await hostTouchSessionAction(sessionId);
    } catch (e) {}
  };
  ...
  useEffect(() => {
    ...
    const completeSessionIfOpen = () => {
      ...
      hostTerminateQuizAction(sessionId).catch(() => {});
    };
  ...
  // Control channel and Pusher events
  useEffect(() => {
    if (!sessionId) return;
    ...
    channel.bind("anti_cheat_violation", (payload: any) => {
      ...
      const violationTimestamp = payload?.timestamp || new Date().toISOString();
    ...
    // Submissions are handled below via interval polling, or we can bind here if needed
    // The previous implementation used postgres_changes on student_responses

    return () => {
      channel.unbind("anti_cheat_violation");
  ...
  // Submissions listener via Pusher
  useEffect(() => {
    ...
    // We bind a specific student submission event
    channel.bind("student-submission", (payload: any) => {
      if (payload.questionId === qId) {
        setSubmissionCount((c) => c + 1);
      }
    });

    return () => {
      channel.unbind("student-submission");
    };
  ...
  useEffect(() => {
    ...
    const refreshSubmissionCount = async () => {
      try {
        const count = await hostGetSubmissionCountAction(sessionId, qId);
  ...
  useEffect(() => {
    ...
    const refreshParticipants = async () => {
      try {
        const data = await hostFetchSessionAction(sessionId);
    ...
    try {
      ...
      setSessionInfo({
        ...
        quizzes: {
          ...
          questions: data.quiz.questions.map((q: any) => ({ id: q.id }))
        }
      });
      
      setQuestions(data.quiz.questions);
      setSessionStatus(data.status.toLowerCase() as any);
      ...
      // Violations not yet in schema - initialize empty map
  ...
  const startGame = async () => {
    ...
    setStartError(null);
    try {
      await hostStartGameAction(sessionId);
  ...
  const nextQuestion = async () => {
    ...
    try {
      await hostAdvanceQuestionAction(sessionId, nextQuestionIndex, isLast);
  ...
  const terminateQuiz = async () => {
    if (!sessionId) return;
    try {
      await hostTerminateQuizAction(sessionId);
  ...
  const kickParticipant = async (pId: string, pName: string) => {
    ...
    try {
      await hostKickParticipantAction(sessionId, pId);
  ...
  const handleOpenViolations = async (pId: string, pName: string) => {
    ...
    try {
      const dbData = await hostFetchViolationsAction(sessionId, pId);
  ...
  useEffect(() => {
    ...
    participantsList.forEach((p: any) => {
      ...
      hostLogViolationAction(sessionId, p.id, violationType).catch(() => {});
    });
