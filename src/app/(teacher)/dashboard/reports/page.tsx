...
export default function ReportsDashboard() {
  ...
  useEffect(() => {
    ...
    fetchFinishedSessions(session.user.id);
  }, [session]);

  const fetchFinishedSessions = async (userId: string) => {
    try {
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {sessions.map((sess, idx) => {
            ...
            const totalCheats = sortedPlayers.reduce((sum, p) => sum + (p.cheatFlags || 0), 0);
            ...
            return (
              ...
              >
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 border-b border-gray-100 dark:border-white/10 pb-6">
                  <div>
                    <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-1">{sess.quiz?.title || "Unknown Quiz"}</h3>
                    <p className="text-sm font-medium text-slate-500">
                      {sess.status === "ACTIVE" ? "Live now" : "Played on"} {new Date(sess.finishedAt || sess.startedAt || Date.now()).toLocaleString()}
                    </p>
                  </div>
                    ...
                    {sortedPlayers.slice(0, 5).map((p, i) => (
                      <div key={p.id} className="flex justify-between items-center bg-gray-50 dark:bg-slate-900 p-4 rounded-xl border border-gray-100 dark:border-white/5">
                        <div className="flex items-center gap-4">
                          ...
                          <span className="font-bold text-slate-800 dark:text-slate-200">{p.displayName}</span>
                          {p.cheatFlags > 0 && <span className="text-xs bg-rose-100 text-rose-600 px-2 py-0.5 rounded-md font-bold text-center">FLAGGED ×{p.cheatFlags}</span>}
                        </div>
