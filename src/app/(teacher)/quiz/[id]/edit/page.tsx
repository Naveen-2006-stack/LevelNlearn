...
import { fetchQuizEditorData, saveQuizData, deleteQuestionAction, uploadQuizImage } from "@/actions/quiz";
...
const normalizeQuestionType = (q: any): QuestionType => {
  if (q?.questionType === "true_false" || q?.questionType === "multi_select" || q?.questionType === "mcq") {
...
export default function QuizEditor() {
  ...
  const fetchQuizData = async () => {
    try {
      const data = await fetchQuizEditorData(quizId);
      setQuiz({ 
        id: data.id, 
        title: data.title, 
        description: data.description || "", 
        timer_based_marking: true, // legacy prop
        test_mode: false 
      });
      ...
      setSaveStatus({ type: "error", message: err.message || "Error loading quiz" });
    }
  };
  ...
  const uploadQuestionImageLocal = async (qIndex: number, file?: File | null) => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const imageUrl = await uploadQuizImage(quizId, q.id, base64Data, file.type);
        updateQuestion(qIndex, "image_url", imageUrl);
      ...
      setSaveStatus({ type: "error", message: err.message || `Question ${qIndex + 1}: failed to upload image.` });
      setUploadingQuestionId(null);
    }
  };
  ...
  const deleteQuestion = async (index: number, rawId: string, isNew?: boolean) => {
    if (!isNew) {
      await deleteQuestionAction(rawId);
    }
  ...
  const saveQuiz = async () => {
    ...
    try {
      await saveQuizData(quizId, quiz, questions);
      await fetchQuizData();
                ...
                <div className="flex-1 space-y-4">
                      ...
                      <label className="inline-flex items-center gap-2 cursor-pointer rounded-xl bg-white px-4 py-2 text-sm font-semibold text-indigo-700 border border-indigo-200 hover:bg-indigo-50 transition-colors">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            void uploadQuestionImageLocal(qIdx, file);
                  ...
                  <div className="flex flex-wrap gap-4">
                    <div className="flex items-center gap-2 bg-gray-50 dark:bg-slate-900 px-4 py-2 rounded-xl border border-gray-200 dark:border-white/10">
                      ...
                      >
                        <option value="mcq" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">Normal</option>
                        <option value="multi_select" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">Multiple Choice</option>
