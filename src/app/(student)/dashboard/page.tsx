...
import { fetchProfile, fetchHistory, fetchQuizzes, fetchReports, createNewQuizAction, deleteQuizAction, startSessionAction } from "@/actions/dashboard";
...
export default function UnifiedDashboard() {
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      ...
      loadData(session.user.id as string);
    }
  ...
  const loadData = async (userId: string) => {
    ...
    fetchHistory(userId, deviceUuid).then((data: any) => {
      setHistory(data);
      setLoadingHistory(false);
    });

    fetchQuizzes(userId).then(data => {
      setQuizzes(data as any);
      setLoadingQuizzes(false);
    });

    fetchReports(userId).then(data => {
      setReportSessions(data as any);
  ...
  const createNewQuiz = async () => {
    ...
    const newQuizId = await createNewQuizAction(session.user.id as string);
  ...
  const startSession = async (quizId: string) => {
    ...
    const sessionId = await startSessionAction(quizId, session.user.id as string, joinCode);
    if (sessionId) router.push(`/host/${sessionId}`);
  };
